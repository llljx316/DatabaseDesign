...
class ShipRoutePoint(models.Model):
    id = models.AutoField(primary_key=True)  # 船只ID
...
class EditShipView(models.Model):
    ship = models.ForeignKey(Ship,on_delete=models.CASCADE)
    user = models.ForeignKey(newuser,on_delete=models.CASCADE)
    edittime = models.DateTimeField(auto_now=True)
    operation = models.CharField(max_length=6,null=False,choices=[('edit', 'Edit'), ('create', 'Create')])

class EditShipPointView(models.Model):
    point = models.ForeignKey(ShipRoutePoint,on_delete=models.CASCADE)
    user = models.ForeignKey(newuser,on_delete=models.CASCADE)
    edittime = models.DateTimeField(auto_now=True)
