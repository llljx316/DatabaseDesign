<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="船ID" width="95">
        <template slot-scope="scope">
          {{ scope.row.ship.shipid }}
        </template>
      </el-table-column>
      <el-table-column label="船只名称">
        <template slot-scope="scope">
          {{ scope.row.ship.name }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      GetEditShips().then(response => {
        this.list = response.data.results
        this.listLoading = false
      })
    },
    handleSearchUser() {
      SearchEditShips('user__username', this.searchTextUser).then(res => {
        console.log(res)
        this.list = res.data.results
      })
    },
    handleSearchShip() {
      SearchEditShips('ship__name', this.searchTextShip).then(res => {
        console.log(res)
        this.list = res.data.results
      })
    },
    handleSortChange({ column, prop, order }) {
      if (prop === 'edittime') {
        this.tableData.sort((a, b) => {
          return order === 'ascending' ? new Date(a.edittime) - new Date(b.edittime) : new Date(b.edittime) - new Date(a.edittime);
        });
      }
    }
  }
